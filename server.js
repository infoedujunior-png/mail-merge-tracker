...
async function getAccessToken(userEmail) {
  ...
  if (user.refreshToken) {
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        // Refresh token revoked — clear it and stop retrying
        if (data.error === 'invalid_grant') {
          console.error(`❌ Refresh token revoked for ${userEmail} — clearing`);
  ...
  // No refresh token — use access token if < 58 min old (extended from 50)
  if (user.accessToken) {
    ...
    console.log(`⚠️ Token expired for ${userEmail} (${ageMin} min old) — needs refresh token for long schedules`);
    return null;
  }
